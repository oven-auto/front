...
node-install:
	sudo docker exec front-node npm install

#собрать пакеты под js и css без уменьшения
node-dev:
	sudo docker exec front-node yarn run dev

#собрать пакеты под js и css с уменьшением
node-production:
	sudo docker exec front-node yarn run production
node-watch:
	sudo docker exec front-node yarn run watch
